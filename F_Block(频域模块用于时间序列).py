...
"""
时序数据分析的复杂性极大地受益于时域和频域表示提供的独特优势。虽然时域在表示局部依赖性方面更胜一筹，特别是在非周期序列中，但频域在捕获全局依赖性方面表现出色，使其成为具有明显周期模式的序列的理想选择。
为了利用这两个优势，我们提出了ATFNet，这是一个创新的框架，它结合了时域模块和频域模块，以同时捕获时间序列数据中的局部和全局依赖关系。
具体来说，我们引入了主谐波级数能量加权，这是一种基于输入时间序列的周期性动态调整两个模块之间权重的新机制。
在频域模块中，我们通过扩展DFT增强了传统的离散傅里叶变换（DFT），旨在解决离散频率错位的挑战。
"""
